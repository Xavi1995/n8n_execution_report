...
def format_node_table(pdf, nodes, title):
    ...
    pdf.ln()
    for node in nodes:
        pdf.cell(60, 8, node['name'][:30], border=1)
        pdf.cell(60, 8, node['type'].split('.')[-1], border=1)
        pdf.cell(30, 8, str(node.get('execution_time', 0)), border=1)
        pdf.ln()
    pdf.ln()
...
def process_workflow(pdf, workflow, level=0):
    ...
    if ai_nodes:
        ...
        for ai in ai_nodes:
            ...
            pdf.cell(50, 8, ai['name'][:25], border=1)
            pdf.cell(40, 8, details.get('model_used', 'N/A'), border=1)
            tokens = f"{details.get('prompt_tokens', 0)} + {details.get('completion_tokens', 0)}"
            pdf.cell(30, 8, tokens, border=1)
            pdf.cell(30, 8, str(ai.get('execution_time', 0)), border=1)
            pdf.cell(30, 8, details.get('cost_execution', 'N/A'), border=1)
            pdf.ln()
        pdf.ln()
